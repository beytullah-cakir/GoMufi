...
const ContextMenu: React.FC<ContextMenuProps> = ({
          ...
          <div className="relative">
            ...
            >
              <div
                ...
                style={{ backgroundColor: slideBackgroundColor || "#ffffff" }}
              />
            </button>
  ...
  const allText = elements.every((el) =>
    ["text", "sticky", "shape"].includes(el.type),
  ); // Removed code from here
  const isSingle = elements.length === 1;
  const hasMedia = elements.some((el) => ["image", "video"].includes(el.type));
  const allBorders = elements.every((el) =>
      ...
      <div className="bg-gray-900 text-white p-2 rounded-xl shadow-2xl flex items-center gap-2 border border-gray-700 select-none">
        ...
        {allCode && (
          <div className="flex bg-gray-800 rounded border border-gray-600 p-0.5">
            <button
              onClick={() =>
                updateElement(firstEl.id, {
                  codeConfig: { ...firstEl.codeConfig, theme: "dark" },
                })
              }
            ...
            <button
              onClick={() =>
                updateElement(firstEl.id, {
                  codeConfig: { ...firstEl.codeConfig, theme: "light" },
                })
              }
        ...
        {(allText || allCode) && (
          <select
            ...
            onChange={(e) =>
              bulkUpdateStyle({ fontFamily: e.target.value as any })
            }
            onMouseDown={(e) => e.stopPropagation()}
          >
            {!allCode && (
              ...
              onMouseDown={(e) => {
                ...
                elements.forEach((el) =>
                  updateElementStyle(el.id, {
                    fontSize: Math.max(8, (el.style?.fontSize || 24) - 2),
                  }),
                );
              }}
              className="w-6 h-8 hover:bg-gray-700 flex items-center justify-center"
            >
              -
            </button>
            <span className="w-8 text-center text-xs tabular-nums">
              {firstEl.style?.fontSize || 24}
            </span>
            <button
              onMouseDown={(e) => {
                e.stopPropagation();
                elements.forEach((el) =>
                  updateElementStyle(el.id, {
                    fontSize: (el.style?.fontSize || 24) + 2,
                  }),
                );
              }}
              className="w-6 h-8 hover:bg-gray-700 flex items-center justify-center"
        ...
        {(allText || allCode) && (
          <div className="flex gap-0.5">
            <button
              ...
              className={`p-1.5 rounded hover:bg-gray-700 ${firstEl.style?.bold ? "text-yellow-400 bg-gray-800" : "text-gray-300"}`}
            >
            ...
            <button
              ...
              className={`p-1.5 rounded hover:bg-gray-700 ${firstEl.style?.italic ? "text-yellow-400 bg-gray-800" : "text-gray-300"}`}
            >
            ...
            <button
              ...
              className={`p-1.5 rounded hover:bg-gray-700 ${firstEl.style?.underline ? "text-yellow-400 bg-gray-800" : "text-gray-300"}`}
            >
          ...
          <div className="relative">
            <button
              onClick={(e) => {
                e.stopPropagation();
                setActiveColorPickerId(
                  activeColorPickerId === "text-color" ? null : "text-color",
                );
              }}
              onMouseDown={(e) => {
                e.preventDefault();
              ...
              title="Text Color"
            >
              <span className="font-serif font-bold text-lg leading-none text-gray-200">
                A
              </span>
              <div
                ...
                style={{
                  backgroundColor:
                    firstEl.style?.color || (allCode ? "#d4d4d4" : "black"),
                }}
              />
            </button>
        ...
        {elements.every(
          (el) =>
            ["shape", "sticky", "circle"].includes(el.type) ||
          ...
          <div className="relative">
            <button
              onClick={(e) => {
                e.stopPropagation();
                setActiveColorPickerId(
                  activeColorPickerId === "fill-color" ? null : "fill-color",
                );
              }}
              onMouseDown={(e) => {
                e.preventDefault();
              ...
              style={{
                backgroundColor:
                  firstEl.style?.backgroundColor || "transparent",
              }}
              title="Fill Color"
            />
            ...
            {activeColorPickerId === "fill-color" && (
              <div className="absolute top-full left-1/2 -translate-x-1/2 pt-3 z-[1001]">
                <ColorPicker
                  ...
                  onUpdate={(color) =>
                    bulkUpdateStyle({ backgroundColor: color })
                  }
                  setActiveColorPickerId={setActiveColorPickerId}
                />
              </div>
          ...
          <>
            ...
            <div className="relative">
              ...
              {isAlignMenuOpen && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 pt-3 z-[1001]">
                  <div
                    className="bg-gray-900 p-2 rounded-xl shadow-xl border border-gray-600 flex flex-col gap-2 w-32"
                    onMouseDown={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                    }}
                  >
                    <div className="flex justify-between bg-gray-800 rounded p-1">
                      <button
                        ...
                        onClick={() => bulkUpdateStyle({ textAlign: "left" })}
                        className={`p-1 rounded hover:bg-gray-700 ${firstEl.style?.textAlign === "left" ? "text-indigo-400" : "text-gray-400"}`}
                      >
                        <AlignLeft className="w-4 h-4" />
                      </button>
                      <button
                        onMouseDown={(e) => e.preventDefault()}
                        onClick={() => bulkUpdateStyle({ textAlign: "center" })}
                        className={`p-1 rounded hover:bg-gray-700 ${!firstEl.style?.textAlign || firstEl.style?.textAlign === "center" ? "text-indigo-400" : "text-gray-400"}`}
                      >
                        <AlignCenter className="w-4 h-4" />
                      </button>
                      <button
                        onMouseDown={(e) => e.preventDefault()}
                        onClick={() => bulkUpdateStyle({ textAlign: "right" })}
                        className={`p-1 rounded hover:bg-gray-700 ${firstEl.style?.textAlign === "right" ? "text-indigo-400" : "text-gray-400"}`}
                      >
          ...
          <>
            ...
            <div className="relative">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setIsBorderMenuOpen(!isBorderMenuOpen);
                  setActiveColorPickerId(null);
                }}
                onMouseDown={(e) => {
                  e.preventDefault();
              ...
              {isBorderMenuOpen && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 pt-3 z-[1001]">
                  <div
                    className="bg-gray-900 p-3 rounded-xl shadow-xl border border-gray-600 flex flex-col gap-3 w-52"
                    onMouseDown={(e) => e.stopPropagation()}
                  >
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <input
                        ...
                        max="50"
                        value={
                          firstEl.style?.borderRadius ??
                          (firstEl.type === "shape" &&
                          firstEl.shapeType === "circle"
                            ? 50
                            : 0)
                        }
                        onChange={(e) =>
                    ...
                    <div className="flex bg-gray-800 p-0.5 rounded-lg border border-gray-700">
                      <button
                        onClick={() =>
                          bulkUpdateStyle({ borderPosition: "inside" })
                        }
                      ...
                      <button
                        onClick={() =>
                          bulkUpdateStyle({ borderPosition: "outside" })
                        }
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="relative">
                        <button
                          ...
                          style={{
                            backgroundColor:
                              firstEl.style?.borderColor || "transparent",
                          }}
                        />
                        {activeColorPickerId === "multi-border" && (
        ...
        {isSingle && hasMedia && (
          <input
            ...
            value={
              firstEl.type === "image"
                ? firstEl.imageUrl || firstEl.src || ""
                : firstEl.videoUrl || firstEl.src || ""
            }
            onChange={(e) => {
              if (firstEl.type === "image") {
                updateElement(firstEl.id, { imageUrl: e.target.value });
              } else {
                updateElement(firstEl.id, { videoUrl: e.target.value });
              }
            }}
            onMouseDown={(e) => e.stopPropagation()}
            ...
            <div className="flex gap-0.5">
              ...
              <button
                onClick={() =>
                  elements.forEach((el) =>
                    updateElement(el.id, {
                      arrowConfig: { ...el.arrowConfig!, arrowStyle: "curved" },
                    }),
                  )
                }
              ...
              <button
                onClick={() =>
                  elements.forEach((el) =>
                    updateElement(el.id, {
                      arrowConfig: { ...el.arrowConfig!, arrowStyle: "elbow" },
                    }),
                  )
                }
