...
import { PenTool, HelpCircle, Code, FileText, Image, File, Check, Star } from 'lucide-react';
            ...
            <div className="max-w-5xl mx-auto w-full p-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-5 flex flex-col gap-6">
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`p-4 rounded-2xl border-2 text-left flex flex-col justify-between transition-all group ${
                                                isSelected 
                                                    ? `border-${type.color}-500 bg-${type.color}-50/30 text-${type.color}-700 shadow-md ring-1 ring-${type.color}-400/20` 
                                                    : 'border-gray-100 hover:border-gray-200 hover:bg-gray-50/50 text-gray-500'
                                            }`}
                                        >
                                            <div className="flex items-center justify-between w-full">
                                                <div className={`w-8 h-8 rounded-xl bg-gray-50 group-hover:scale-105 transition-transform flex items-center justify-center ${isSelected ? `bg-${type.color}-100 text-${type.color}-600` : 'text-gray-400'}`}>
                                                    {type.icon}
                                                </div>
                                                {isSelected && (
                                                    <div className={`w-5 h-5 rounded-full bg-${type.color}-500 text-white flex items-center justify-center`}>
                                                        <Check size={12} strokeWidth={3} />
                                                    </div>
                                                )}
                                            </div>
                                            <div className="mt-4">
                                                <h5 className={`text-xs font-black uppercase tracking-wider ${isSelected ? `text-${type.color}-700` : 'text-gray-700'}`}>
                                                    {type.label}
                                                </h5>
                        ...
                        <div className="bg-gradient-to-br from-blue-550 to-indigo-600 rounded-[2rem] p-6 text-white shadow-lg relative overflow-hidden">
                            ...
                            <div className="p-4 bg-white/10 backdrop-blur-md rounded-2xl border border-white/10 text-xs font-medium space-y-2">
                                ...
                                <div className="flex justify-between items-center text-[10px] text-blue-100 font-bold pt-2 border-t border-white/10">
                                    <span>Tür: {config.submissionType.toUpperCase()}</span>
