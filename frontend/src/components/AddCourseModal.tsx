...
import { X, Book, Code, Globe, Music, Palette, Plus, Trash2, ChevronRight, Layout, List, Layers } from 'lucide-react';
...
const AddCourseModal: React.FC<AddCourseModalProps> = ({ isOpen, onClose, onSave, initialData }) => {
    const [activeTab, setActiveTab] = useState<'general' | 'details' | 'curriculum'>('general');
    ...
    useEffect(() => {
        if (isOpen) {
            if (initialData) {
                ...
                setLearningOutcomes(initialData.learningOutcomes?.length ? initialData.learningOutcomes : ['']);
                setRequirements(initialData.requirements?.length ? initialData.requirements : ['']);
                setSections(initialData.curriculum?.length ? initialData.curriculum : [{ id: '1', title: 'Giriş', lectures: [] }]);
            } else {
                // Reset
                setTitle('');
                setSelectedCategory('');
    ...
    const categories = [
        ...
        { id: 'web', label: 'Web Geliştirme', icon: <Globe size={20} />, color: 'purple' },
        { id: 'design', label: 'Tasarım', icon: <Palette size={20} />, color: 'orange' },
    ...
    const addSection = () => {
        setSections([...sections, { id: `sec_${Date.now()}`, title: '', lectures: [] }]);
    };
    ...
    const removeSection = (id: string) => {
        if (confirm('Bu bölümü ve içindeki dersleri silmek istediğinize emin misiniz?')) {
    ...
    const addLecture = (sectionId: string) => {
        setSections(sections.map(s => {
            if (s.id === sectionId) {
                return {
                    ...s,
                    lectures: [...s.lectures, { id: `lec_${Date.now()}`, title: '', duration: '' }]
                };
            }
            return s;
        }));
    ...
    const updateLecture = (sectionId: string, lectureId: string, field: keyof Lecture, value: string) => {
        setSections(sections.map(s => {
            if (s.id === sectionId) {
                return {
                    ...s,
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm transition-opacity animate-fade-in">
            <div className="bg-white rounded-3xl w-full max-w-4xl h-[90vh] shadow-2xl overflow-hidden flex flex-col">
                ...
                <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-10">
                    <div>
                        <h3 className="text-2xl font-black text-gray-800">{initialData ? 'Kursu Düzenle' : 'Yeni Kurs Oluştur'}</h3>
                        <p className="text-gray-500 font-medium text-sm">Harika bir eğitim deneyimi tasarlayın</p>
                    </div>
                ...
                <div className="flex border-b border-gray-100 px-8">
                    <button
                        ...
                        className={`py-4 px-6 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'general' ? 'border-sky-500 text-sky-600' : 'border-transparent text-gray-400 hover:text-gray-600'}`}
                    >
                        <Layout size={18} />
                        Genel Bilgiler
                    </button>
                    <button
                        ...
                        className={`py-4 px-6 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'details' ? 'border-sky-500 text-sky-600' : 'border-transparent text-gray-400 hover:text-gray-600'}`}
                    >
                        <List size={18} />
                    ...
                    <button
                        ...
                        className={`py-4 px-6 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'curriculum' ? 'border-sky-500 text-sky-600' : 'border-transparent text-gray-400 hover:text-gray-600'}`}
                    >
                        <Layers size={18} />
                        Müfredat
                    </button>
                ...
                <div className="flex-1 overflow-y-auto p-8 bg-gray-50">
                    <form id="courseForm" onSubmit={handleSubmit} className="space-y-8 max-w-3xl mx-auto">
                        ...
                        {activeTab === 'details' && (
                            <div className="space-y-8 animate-fade-in">
                                ...
                                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                    ...
                                    <p className="text-sm text-gray-500 mb-4">Öğrencilerin bu kursu tamamladıktan sonra kazanacağı yetenekleri listeleyin.</p>
                                        ...
                                        {learningOutcomes.map((item, idx) => (
                                            <div key={idx} className="flex gap-2">
                                                <input
                                                    type="text"
                                                    value={item}
                                                    onChange={(e) => handleListChange(idx, e.target.value, learningOutcomes, setLearningOutcomes)}
                                                ...
                                                {learningOutcomes.length > 1 && (
                                                    <button
                                                        type="button"
                                                        onClick={() => removeListItem(idx, learningOutcomes, setLearningOutcomes)}
                                ...
                                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                    ...
                                    <p className="text-sm text-gray-500 mb-4">Bu kursa başlamadan önce öğrencilerin sahip olması gereken bilgi veya ekipmanlar.</p>
                                        ...
                                        {requirements.map((item, idx) => (
                                            <div key={idx} className="flex gap-2">
                                                <input
                                                    type="text"
                                                    value={item}
                                                    onChange={(e) => handleListChange(idx, e.target.value, requirements, setRequirements)}
                                                ...
                                                {requirements.length > 1 && (
                                                    <button
                                                        type="button"
                                                        onClick={() => removeListItem(idx, requirements, setRequirements)}
                                                        className="p-3 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors"
                                                    >
                                                        <Trash2 size={20} />
                                                    </button>
                        ...
                        {activeTab === 'curriculum' && (
                            <div className="space-y-6 animate-fade-in pb-20">
                                <div className="flex justify-between items-center mb-2">
                                    <h4 className="text-lg font-black text-gray-800">Ders Programı</h4>
                                    <button
                                        type="button"
                                        onClick={addSection}
                                ...
                                {sections.map((section, sIdx) => (
                                    <div key={section.id} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                                        ...
                                        <div className="bg-gray-50 p-4 border-b border-gray-200 flex items-center gap-3">
                                            <span className="font-bold text-gray-400 text-sm">Bölüm {sIdx + 1}:</span>
                                        ...
                                        <div className="p-4 space-y-3">
                                            {section.lectures.map((lecture, lIdx) => (
                                                <div key={lecture.id} className="flex items-center gap-3 pl-4 border-l-2 border-gray-100">
                                                    <span className="text-xs font-bold text-gray-300 w-6">{lIdx + 1}.</span>
                ...
                <div className="p-6 border-t border-gray-100 bg-white flex justify-between items-center z-10">
                    <div className="text-sm font-bold text-gray-400">
                        {activeTab === 'general' ? '1/3 Genel Bilgiler' : activeTab === 'details' ? '2/3 Detaylar' : '3/3 Müfredat'}
                    </div>
                    <div className="flex gap-3">
                        ...
                        {activeTab !== 'curriculum' ? (
                            <button
                                type="button"
                                onClick={() => setActiveTab(activeTab === 'general' ? 'details' : 'curriculum')}
