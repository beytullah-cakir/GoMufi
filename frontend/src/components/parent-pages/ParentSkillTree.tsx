...
interface ParentSkillTreeProps {
  userData?: any;
}

const ParentSkillTree: React.FC<ParentSkillTreeProps> = ({ userData }) => {
  const students = userData?.students || [];
  const studentName =
    students.length > 0
      ? students[0].first_name || students[0].nickname
      : "Çocuğunuz";
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "completed":
        return "bg-green-500 text-white shadow-green-200 border-green-400";
      case "in_progress":
        return "bg-yellow-500 text-white shadow-yellow-200 border-yellow-400";
      default:
        return "bg-gray-100 text-gray-400 border-gray-200 shadow-none"; // locked
    }
  };
      ...
      <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium max-w-xl">
            {studentName} öğrenme yolculuğuna burada devam ediyor. Becerilerini
            bir ağaç gibi daldan dala büyütüyor.
          </p>
        </div>
            ...
            <div className="space-y-8 relative z-10">
              {skills.math.map((skill) => (
                <div key={skill.id} className="flex items-center gap-6 group">
                  {/* Node */}
                  <div
                    className={`w-16 h-16 rounded-2xl flex items-center justify-center border-4 shadow-lg transition-transform hover:scale-110 z-10 ${getStatusColor(skill.status)}`}
                  >
                  ...
                  {/* Card */}
                  <div
                    className={`flex-1 p-4 rounded-2xl border-2 transition-all ${skill.status === "locked" ? "bg-gray-50 border-gray-100 opacity-60" : "bg-white border-gray-100 hover:border-purple-200 hover:shadow-md"}`}
                  >
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-bold text-gray-800">{skill.name}</h4>
                      {skill.status !== "locked" && (
                        <span
                          className={`text-xs font-bold px-2 py-1 rounded-lg ${skill.score >= 90 ? "bg-green-100 text-green-600" : "bg-yellow-100 text-yellow-600"}`}
                        >
                          {skill.score}%
                        </span>
                      )}
            ...
            <div className="space-y-4">
              <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/20">
                <p className="font-medium text-purple-50 leading-relaxed italic">
                  "Çocuğunuz görsel hafızayı kullanan problemlerde çok daha
                  başarılı. Geometri konularına geçiş yaptığımızda
                  performansının artacağını öngörüyoruz."
                </p>
              ...
              <button className="w-full text-left p-4 rounded-2xl bg-blue-50 hover:bg-blue-100 border border-blue-100 transition-colors group">
                <div className="flex justify-between items-center mb-1">
                  <span className="font-bold text-blue-700">
                    Geometri Seti İncele
                  </span>
                  <ChevronRight className="w-5 h-5 text-blue-400 group-hover:translate-x-1 transition-transform" />
                </div>
                <p className="text-xs text-blue-600 font-medium">
                  Görsel zekayı destekleyen materyaller.
                </p>
              </button>
            </div>
          </div>
