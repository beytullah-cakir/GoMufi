...
const InstructorLayout: React.FC<InstructorLayoutProps> = ({ activePage, onNavigate, children }) => {
    return (
                ...
                <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-gray-200 px-8 py-4 flex justify-between items-center">
                    <div className="flex items-center gap-8 flex-1">
                        <h1 className="text-2xl font-black text-gray-800 tracking-tight whitespace-nowrap min-w-[150px]">{
                            activePage === 'Dashboard' ? 'Panel' :
                                activePage === 'Courses' ? 'Kurslarım' :
                                    activePage === 'Content' ? 'İçerik Yönetimi' :
                                        activePage === 'Students' ? 'Öğrenciler' :
                                            activePage === 'Interactions' ? 'Etkileşimler' :
                                                activePage === 'Assessments' ? 'Değerlendirme' :
                                                    activePage === 'Calendar' ? 'Takvim' :
                                                        activePage === 'Revenue' ? 'Gelir' :
                                                            activePage === 'Settings' ? 'Ayarlar' : activePage
                        }</h1>
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex items-center gap-3 pl-6 border-l-2 border-gray-100">
                            <div className="text-right hidden md:block">
                                <p className="text-sm font-black text-gray-800">Mualla Hoca</p>
