...
type BattlePhase = 'intro' | 'player_turn' | 'question' | 'monster_turn' | 'effect' | 'victory' | 'defeat';

const MonsterBattleGame: React.FC<MonsterBattleGameProps> = ({ level, onClose, onComplete }) => {
    // Game State
    ...
    if (phase === 'intro') {
        return (
            <div className="flex flex-col items-center justify-center h-full animate-in zoom-in-50 bg-black text-white">
                <h1 className="text-6xl font-black font-display mb-8">Vahşi CANAVAR belirdi!</h1>
            </div>
        );
    }
    ...
    return (
        ...
        >
            ...
            <div className="w-full h-full max-w-5xl mx-auto relative">
                ...
                <div className="absolute top-12 right-12 flex flex-col items-end">
                    ...
                    <div className="bg-white/90 border-4 border-gray-700 rounded-lg p-4 mb-4 shadow-lg min-w-[300px] skew-x-[-12deg]">
                        <div className="skew-x-[12deg]">
                            <h2 className="text-xl font-black text-gray-800 uppercase">Karanlık Ejderha</h2>
                            <div className="w-full bg-gray-300 h-4 rounded-full mt-2 border border-gray-500 overflow-hidden relative">
                                <div className="absolute top-0 right-0 w-full h-full bg-gray-300"></div> {/* Background fix */}
                                <div
                                    className={`h-full transition-all duration-500 ${monsterHP < 50 ? 'bg-red-500' : 'bg-green-500'}`}
                    ...
                    <div className="relative mr-16">
                        <div className="absolute -bottom-8 -left-8 w-48 h-12 bg-black/20 rounded-[50%] blur-md"></div> {/* Shadow */}
                        <img
                            src={dragonSprite}
                            alt="Monster"
                ...
                <div className="absolute bottom-40 left-12 flex flex-col items-start">
                    ...
                    <div className="relative ml-16 mb-4">
                        <div className="absolute -bottom-6 -left-4 w-40 h-10 bg-black/20 rounded-[50%] blur-md"></div> {/* Shadow */}
                        <img
                            src={mufiArmorSprite}
                            alt="Hero"
                    ...
                    <div className="bg-white/90 border-4 border-gray-700 rounded-lg p-4 shadow-lg min-w-[320px] skew-x-[-12deg]">
                        <div className="skew-x-[12deg]">
                            <div className="flex justify-between items-end mb-1">
                                <h2 className="text-xl font-black text-gray-800 uppercase">Kahraman Mufi</h2>
                                <span className="text-lg font-bold text-gray-600">{playerHP}/{maxPlayerHP}</span>
                            </div>
                            <div className="w-full bg-gray-300 h-4 rounded-full border border-gray-500 overflow-hidden relative">
                                <div
                                    className={`h-full transition-all duration-500 ${playerHP < 30 ? 'bg-red-500' : 'bg-green-500'}`}
                ...
                {phase !== 'question' && (
                    <div className="absolute bottom-4 right-4 max-w-xl w-full flex gap-4 p-4 bg-gray-800/80 backdrop-blur-md rounded-t-3xl border-t-4 border-white/20">
                        <div className="grid grid-cols-2 gap-4 w-full">
                            ...
                            >
                                İyileş <span className="text-sm bg-black/20 px-2 rounded">{inventory.potions}</span>
                            </button>
                            <button
                            ...
                            >
                                Savun <span className="text-sm bg-black/20 px-2 rounded">{inventory.shields}</span>
                            </button>
                            <button
                            ...
                            >
                                Bomba <span className="text-sm bg-black/20 px-2 rounded">{inventory.bombs}</span>
                            </button>
                        </div>
                ...
                {phase === 'question' && (
                    <div className="absolute bottom-0 left-0 right-0 h-[40%] bg-white border-t-8 border-gray-800 p-8 flex flex-col animate-slide-up">
                        <h3 className="text-2xl font-black text-gray-800 mb-6">{currentQuestion.text}</h3>
            ...
            {/* Close Button */}
            <button onClick={onClose} className="absolute top-4 left-4 p-2 bg-white/50 backdrop-blur rounded-xl hover:bg-white/80 shadow-sm border border-gray-200 transition-colors z-50">❌</button>
        </div>
    );
};
