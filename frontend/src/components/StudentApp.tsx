...
function StudentApp() {
  ...
  const [courses, setCourses] = useState<Record<string, CourseData>>(() => {
    ...
    mathNodes.push(
      ...generateLessonNodes(1, 1, false, "Temel Aritmetik", true),
    );
        ...
        <div className="fixed bottom-0 right-0 z-[90] pointer-events-none origin-bottom-right m-0 p-0">
          <div className="relative">
            <div className="absolute top-2 right-12 z-20 flex flex-col items-center">
              <span className="text-3xl font-black text-sky-400 animate-zzz font-display leading-none">
                Z
              </span>
              <span
                className="text-2xl font-black text-sky-400/80 animate-zzz font-display absolute -top-4 -right-4 leading-none"
                style={{ animationDelay: "1s" }}
              >
                z
              </span>
              <span
                className="text-xl font-black text-sky-400/60 animate-zzz font-display absolute -top-8 -right-6 leading-none"
            ...
            <img
              src={MufiSleep}
              alt="Sleeping Mufi"
              className="w-56 animate-breathe drop-shadow-2xl relative z-10 block"
            />
          </div>
        </div>
      )}
